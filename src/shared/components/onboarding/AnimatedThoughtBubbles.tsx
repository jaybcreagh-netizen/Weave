...
const FloatingBubble = ({ phrase, delay, index }: { phrase: string; delay: number; index: number }) => {
  ...
  return (
    <Animated.View
      style={animatedStyle}
      className="bg-primary rounded-2xl py-3 px-5 shadow-md border border-primary-muted"
    >
      <Text className="text-base text-primary-foreground italic font-inter-regular">
        {phrase}
      </Text>
    </Animated.View>
  );
