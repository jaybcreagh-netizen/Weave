...
// Approximate path length for strokeDasharray (calculated from SVG editor)
...
export const LoadingScreen: React.FC<LoadingScreenProps> = ({ visible }) => {
  ...
  useEffect(() => {
    if (visible) {
      ...
      // Animate: draw in (2s) → pause (0.3s) → fade out (0.8s)
      strokeDashoffset.value = withSequence(
        withTiming(0, { duration: 2000, easing: Easing.inOut(Easing.ease) }),
        withDelay(300, withTiming(0, { duration: 0 })) // Hold at fully drawn
      );

      opacity.value = withSequence(
        withTiming(1, { duration: 2000 }), // Visible during draw
        withDelay(300, withTiming(0, { duration: 800, easing: Easing.out(Easing.ease) })) // Slower fade out
      );
    }
  }, [visible]);
