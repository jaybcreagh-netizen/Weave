 ...
 *
 * Prompt-led reflection flow (context -> prompt -> write).
 * Ported from GuidedReflectionModal to live inside the unified sheet.
 */
...
interface PromptedReflectionFlowProps {
...
export function PromptedReflectionFlow({
  prefilledText,
  prefilledFriendIds,
  prefilledWeaveId,
  onSave,
  ...
  useEffect(() => {
    onDirtyChange?.(!!text.trim() && !saving)
  }, [text, saving, onDirtyChange])
  ...
  const loadData = async () => {
    ...
    try {
      const [weaves, allFriends] = await Promise.all([
        getRecentMeaningfulWeaves(5, 72),
        database
      ...
      console.error('[PromptedReflection] Error loading data:', error)
    } finally {
      setLoading(false)
    }
  }
              ...
              <ScrollView className="flex-1 p-4" showsVerticalScrollIndicator={false}>
                ...
                {selectedContext.friendContext.recentWeaves.length > 0 && (
                  <View className="mb-4">
                    ...
                    {selectedContext.friendContext.recentWeaves.slice(0, 3).map((weave) => (
            ...
            {filteredFriends.map(friend => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {isSelected && <Text style={{ color: colors['primary-foreground'], fontSize: 12 }}>✓</Text>}
                  </View>
                </TouchableOpacity>
              )
            })}
