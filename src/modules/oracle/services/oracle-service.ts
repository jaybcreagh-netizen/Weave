...
class OracleService {
    ...
    async ask(question: string, friendIds: string[] = [], additionalContext?: string): Promise<OracleResponse> {
        ...
        trackEvent(AnalyticsEvents.ORACLE_QUESTION_ASKED, { context_tier: session.contextTier, friend_count: friendIds.length, question_length: question.length });
        trackEvent(AnalyticsEvents.ORACLE_RESPONSE_RECEIVED, { response_length: parsed.text.length, has_action: !!parsed.action });
    ...
    async *askStream(question: string, friendIds: string[] = [], additionalContext?: string): AsyncGenerator<string, OracleResponse, unknown> {
        ...
        trackEvent(AnalyticsEvents.ORACLE_QUESTION_ASKED, { context_tier: session.contextTier, friend_count: friendIds.length, question_length: question.length, is_stream: true });
        trackEvent(AnalyticsEvents.ORACLE_RESPONSE_RECEIVED, { response_length: parsed.text.length, has_action: !!parsed.action, is_stream: true });
        // Persist to DB
        await this.persistSession(friendIds[0])
    ...
    async analyzeEntryContext(entry: JournalEntry): Promise<OracleSuggestion[]> {
        ...
        try {
            ...
            try {
                ...
                trackEvent(AnalyticsEvents.ORACLE_LENS_SELECTED, {
                    suggestion_count: suggestions.length,
                    has_signals: !!signalData
                });
    ...
    async synthesizeInsights(signals: InsightSignal[]): Promise<void> {
        ...
        try {
            const prompt = `
You're a thoughtful friend who noticed something about their social patterns.

WHAT YOU NOTICED:
${JSON.stringify(topSignals, null, 2)}

WRITE A BRIEF, CASUAL INSIGHT:
- Headline: MAX 40 characters. Catchy, 3-6 words.
- Body: MAX 150 characters. That's about 1-2 short sentences.
